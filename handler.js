...
function getAdminStatus(participants, targetJid) {
  if (!targetJid || !participants?.length) return false
  const targetNum = extraerNum(targetJid)
  const p = participants.find(p => {
    const pNum = extraerNum(p.id)
    const pLid = extraerNum(p.lid)
    const pTel = p.phoneNumber ? String(p.phoneNumber).replace(/\D/g, '') : ''
    return pNum === targetNum || pLid === targetNum || pTel === targetNum
  })
